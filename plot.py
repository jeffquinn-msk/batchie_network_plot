...
def get_data():
    ...
    drugs_to_drop = np.unique(np.concatenate(
        [subset[subset['Drug 1 TI'] < 0.15].drug1,
            subset[subset['Drug 2 TI'] < 0.15].drug2]))

    subset2 = subset[~subset.drug1.isin(drugs_to_drop) | ~subset.drug2.isin(drugs_to_drop)]

    max_per_group_indices = subset2.groupby(['drug1', 'drug2'])[value].transform("max") == subset2[value]

    # normalize ['MSKRMS-12808 TI'] to [0, 1]
    subset2['weight'] = 1 + (19 * ((subset2[value] - subset2[value].min()) / (subset2[value].max() - subset2[value].min())))
...
def create_network():
    ...
    for drug1, drug2, weight in df[['drug1', 'drug2', 'weight']].itertuples(index=False, name=None):
